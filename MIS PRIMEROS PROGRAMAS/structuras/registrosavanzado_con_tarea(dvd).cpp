...
void ordenar_porcodigo(Sdatos alumnos[20]);
...
ordenar_porcodigo(Lista_alumnos);
clrscr();
cout<<"Los registros leidos son"<<endl;
for (i=1;i<=n;i++)
 {cout<<"\nALUMNO Nø "<<i<<endl;
...
for (i=1;i<=n-1;i++)
 for (j=i+1;j<=n;j++)
  if (strcmp(alu[i].apaterno,alu[j].apaterno)>0)
    {tem=alu[i];
     alu[i]=alu[j];
     alu[j]=tem;
     }
...
void ordenar_por_amaterno(Sdatos alu[20])//alfabeticamente
{int i,j;
Sdatos tem;
for (i=1;i<=n-1;i++)
 for (j=i+1;j<=n;j++)
  if (strcmp(alu[i].amaterno,alu[j].amaterno)>0)
    {tem=alu[i];
     alu[i]=alu[j];
     alu[j]=tem;
     }
...
void ordenar_por_nombre(Sdatos alu[20])//alfabeticamente
{int i,j;
Sdatos tem;
for (i=1;i<=n-1;i++)
 for (j=i+1;j<=n;j++)
...
void ordenar_porcodigo(Sdatos alu[20])
{int i,j;
Sdatos tem;
for (i=1;i<=n-1;i++)
 for (j=i+1;j<=n;j++)
  if (strcmp(alu[i].codigo,alu[j].codigo)>0)
    {tem=alu[i];
     alu[i]=alu[j];
     alu[j]=tem;
     }
