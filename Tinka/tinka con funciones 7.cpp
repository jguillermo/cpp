...
#define max 20001
   ...
   presentacion();
   probabilidades();
   genera(int f[max][8],int nums);
   mostrar(int n[41],int f[max][8],int aci[8],int nums);
   menu();
   aciertos(int n[41],int f[max][8],int aci[8],int nums);
   estadisticas(int n[41],int f[max][8],int aci[8],int nums);
   main(){
      randomize(); menu();}
//***************************************************************
   mensaje(int k){
...
//******************************************************
   resultado(int n[41],int k[8],int r[8]){ 
      ...
      if(k1>=1){gotoxy(30,19);
         for (int hu=1;hu<=k1;hu++){cout<<"      "<<r[hu];}}
      mensaje(k1);
   }
//***********************************
   mayormenor(int k[8],int i){ 
...
//**************************
   int numero(){
      ...
      do{gotoxy(17,12);cout<<"ingrese numero de jugadas  ";cin>>nums;
         if(nums>20000){gotoxy(12,17);cout<<"Maximo 20000 jugadas";
            gotoxy(1,12);clreol();}
...
//*******************************
   genera(int f[max][8],int nums){
      ...
      {por=(h*100)/nums; gotoxy(40,10);cout<<por<<" % ";
         numjuego(n);
         for(int j=1;j<=6;j++)
         {f[h][j]=n[j];
         }
...
//************************************
   mostrar(int n[41],int f[max][8],int aci[8],int nums){
      ...
      cout<<"\n\n\tpresione culaquier tecla para ver estadisticas";
      gotoxy(1,1);cout<<"";
      getch();
   }
//*******************************************************
