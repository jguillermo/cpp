...
cout<<"\Precio de venta sugerido x "<<unidad[0]<<unidad[1]<<" al 50 p de FOB   :  ";
...
preciosugerido,fechacompra.dia,fechacompra.mes,fechacompra.year);
char op2;
do{
...
if (p==-1)mensaje("Codigo no esxiste");
 ...
 fstream otro;
 otro.open("auxproducto.dat",ios::out|ios::binary);
 for(int i=0;i<objproductos.numproductos();i++){
 if(i!=p){
  producto unproducto=objproductos.getproducto(i);
   	otro.write((char *)&unproducto,sizeof(unproducto));}

 }
 otro.close();
  fstream entrada, salida;
		entrada.open("auxproducto.dat", ios::in|ios::binary);
		salida.open("productofinal.dat",ios::out|ios::binary);
    producto uno;
   	while(!entrada.eof()){
     ...
     cout<<"\n\n Eliminacion satisfactoria"; getch();}
