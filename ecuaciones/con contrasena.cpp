...
{ contrasena tt;
 double a[20][21];
 int n,i,np,j,m,y=5,com,xc,yc,cc;
 double fc,bg,te,sm;
 char op,opc;
 char busqueda[30];
 ...
 do{  clrscr();
  titulo();
  tt.setcodigo(codi);
  gotoxy(10,10);cout<<"ingrese contraseña      =   ";cin>>busqueda;
  tt.setbusca(busqueda);
  com=strcmp(tt.getbusca(),tt.getcodigo());
 if (com==0)
 {clrscr();
   introduccion();
  getch();
 do
 {
  ...
  gotoxy(1,24);cout<<"numero de ecuaciones  =  ";cin>>n;
   ...
   //condicional para ecuaciones con error
  if (a[n][n]==0)
      break;
